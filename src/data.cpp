...
Type type_of(const H5::DataType &t) {
  auto cls = t.getClass();
  if (cls == H5T_ENUM) return type_of(t.getSuper());
...
template <class T>
long double get(const uint8_t *p) {
  T v;
  std::memcpy(&v, p, sizeof v);
  return v;
}
template <class T>
void put(uint8_t *p, long double v) {
  T value = static_cast<T>(v);
  std::memcpy(p, &value, sizeof value);
}
long double Array::number(size_t i) const {
  if (i >= size()) throw std::out_of_range("Array index");
  auto p = bytes.data() + i * type.bytes;
#define G(N, T) \
  if (type.name == N) return get<T>(p)
  G("uint8", uint8_t);
  G("int8", int8_t);
  G("uint16", uint16_t);
  G("int16", int16_t);
...
Array Array::convert(const Type &t) const {
  if (t.name == type.name) return *this;
  Array out{t, std::vector<uint8_t>(checked_bytes(size(), t.bytes))};
  for (size_t i = 0; i < size(); ++i) out.set(i, number(i));
  return out;
}
Array Array::gather(const std::vector<size_t> &order) const {
  ...
  for (size_t i = 0; i < order.size(); ++i) {
    if (order[i] >= size()) throw std::runtime_error("Invalid permutation");
